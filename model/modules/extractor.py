...
class ModifiedPWCNet(nn.Module):
    """
    PWC-DC net; just use first few encoding layers. Make it fatter & add batchnorm
    """
    # doesn't use batchnorm!
    def __init__(self):
        super(ModifiedPWCNet,self).__init__()

        self.conv1a  = conv(3,   32, kernel_size=3, stride=2)
        self.conv1aa = conv(32,  32, kernel_size=3, stride=1)
        self.conv1b  = conv(32,  32, kernel_size=3, stride=1)
        self.conv2a  = conv(32,  64, kernel_size=3, stride=2)
        self.conv2aa = conv(64,  64, kernel_size=3, stride=1)
        self.conv2b  = conv(64,  64, kernel_size=3, stride=1)
        self.conv3a  = conv(64,  128, kernel_size=3, stride=2)
...
class ModifiedPWCNetBig(nn.Module):
    """
    PWC-DC net; just use first few encoding layers. Make it fatter & add batchnorm
    """
    # doesn't use batchnorm!
    def __init__(self):
        super(ModifiedPWCNetBig,self).__init__()
        ...
        self.conv4aa = conv(128,  128, kernel_size=3, stride=1)
